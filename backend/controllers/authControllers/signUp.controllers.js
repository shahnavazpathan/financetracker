...
const signUp =  async (req, res) => {
    try {
      ...
      if (username.includes(" ") || password.includes(" ") || email.includes(" ") || confirmPassword.includes(" ")) {
      ...
      if (!/^(?=.*[A-Za-z])(?=.*\d)(?=.*[!@#$%^&*(),.?":{}|<>])[A-Za-z\d!@#$%^&*(),.?":{}|<>]+$/.test(password)) {
        return res.status(400).json({ message: "Invalid password: Must be alphanumeric and contain at least one special character!" });
      ...
      if (userId == undefined) {
        ...
        if (isSent === false) {
        return res.status(500).json({message: "please enter valid email address!"});
         } 
